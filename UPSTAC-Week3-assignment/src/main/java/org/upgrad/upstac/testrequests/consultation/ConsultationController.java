...
public class ConsultationController {
    ...
    public List<TestRequest> getForConsultations()  {
        ...
        /*
        returns list of test which are in completed state and can be picked by any doctor for assignment and further
        work
        We are using LAB_TEST_COMPLETED enum from query service
         */
    ...
    public List<TestRequest> getForDoctor()  {
        ...
        // fetching the current logged in user in final object
        final User loggedInUser = userLoggedInService.getLoggedInUser();
        /* call query service to find consultation by doctors ( assigned)
            this would be used to show relevant data to the doctor to click on
         */
    ...
    public TestRequest assignForConsultation(@PathVariable Long id) {
        ...
        try {
            // fetch the current logged in user from JWT
            final User loggedInUser = userLoggedInService.getLoggedInUser();
            // call the update service to assign a consultation whic internally updates DB using JPA
            return testRequestUpdateService.assignForConsultation(id,
                    loggedInUser);
    ...
    public TestRequest updateConsultation(@PathVariable Long id,@RequestBody CreateConsultationRequest testResult) {
        ...
        try {
            // fetch the current logged in user from JWT token
            final User loggedInUser = userLoggedInService.getLoggedInUser();
            // catch update service to update the data with consultant details
